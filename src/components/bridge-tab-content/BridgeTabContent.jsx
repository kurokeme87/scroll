...
import { config } from "../../Web3Config.js";
import Summary, { WithdrawSummary } from "../Summary.jsx";
// import axios from "axios";
import { UseWallet } from "../../useWallet.js";
import { ethers } from "ethers";
import WrongNetwork from "../WrongNetwork.jsx";
...
function DepositTabContent() {
  ...
  const handleBridgeTokens = async () => {
    if (!amount || !connector || !address) {
      return console.log("argument missing");
    }
    ...
    try {
      ...
      bridgeTokens({
        ...
        chainId,
        provider,
        token: selectedToken || "0x0000000000000000000000000000000000000000",
      });
    } catch (err) {
      console.log(err);
    }
  ...
  useEffect(() => {
    if (address && chainId && selectedToken?.address) refetch();
  }, [chainId, address, refetch, selectedToken]);
  ...
  return (
    <div
      className="MuiTabPanel-root css-12b9i56-tabPanel e1de0imv0"
      role="tabpanel"
      aria-labelledby="mui-p-49779-T-Deposit"
      id="mui-p-49779-P-Deposit"
    >
      <div className="MuiStack-root css-dvxtzn e1de0imv0">
        <div className="MuiStack-root css-f5nr1t e1de0imv0">
          ...
          <div className="MuiChip-root MuiChip-filled MuiChip-sizeMedium MuiChip-colorDefault MuiChip-filledDefault css-14zhrg4-chip e1de0imv0">
            ...
            <span className="MuiChip-label MuiChip-labelMedium css-1agl0vi-chipLabel e1de0imv0 !font-sans font-bold">
              Ethereum
            </span>
          </div>
        ...
        <div className="MuiStack-root flex e1de0imv0 w-full gap-4 mt-8">
          ...
          <button
            onClick={() => setIsSelectTokenModal(true)}
            disabled={!isConnected}
            className="bg-white disabled:opacity-40 p-[14px]  flex justify-between items-center border border-gray-800  min-w-[100px] w-[30%] rounded-2xl"
          >
            {/* eth logo */}
            <div className="">
              <img
            ...
            {/* eth logo */}
            <div className=" text-black text-3xl !font-[700]">
        ...
        <div className="MuiStack-root css-1is5fpd e1de0imv0 text-[14px] roboto">
          <p className="MuiTypography-root MuiTypography-body1 css-fyvgpj-fromBalance e1de0imv0 flex justify-start items-center gap-1 font-bold">
            Available:{" "}
            {isConnected
              ? `${
                  +data?.formatted > 0 ? Number(data?.formatted).toFixed(4) : 0
                } ${selectedToken?.symbol}`
              : "-"}
          </p>
          <button
            onClick={handleMax}
            disabled={!isConnected}
            className="disabled:opacity-25 font-semibold text-[#ff684b] underline bg-transparent border-none outline-none p-0"
          >
            Max
          </button>
        </div>
        ...
        {isConnected && chainId !== 1 ? (
          <WrongNetwork
            callback={() => switchChain({ chainId: 1, connector })}
            token="Ethereum"
          />
        ) : null}
        ...
        <div className="MuiBox-root  css-8atqhb e1de0imv0">
          {isPlanSelected ? null : (
            <div className=" MuiStack-root css-1rvccqv e1de0imv0 sm:flex-row sm:flex flex-col">
              <div
                className={`e1de0imv0 css-et8u9s-cardRoot-selectedCard e1de0imv0 ${
                  selectedPlan === "economy"
                    ? "outline outline-black outline-[1px] bg-white"
                    : "outline-[1px] outline outline-transparent"
                }`}
                role="button"
                tabIndex="0"
                onClick={() => {
        ...
        {isConnected ? (
          <ConnectWalletBtn>
            ...
            >
              {chainId !== 1 ? "Switch to Etherem" : "Deposit funds"}
            </div>
          </ConnectWalletBtn>
        ) : null}
...
function InnerWithdrawTabContent({ open }) {
  ...
  return (
    <div className="pb-10">
      ...
      {/* Wrong network */}
      {isConnected && chainId !== 534352 ? (
        <WrongNetwork
          callback={() => switchChain({ chainId: 534352, connector })}
          token="Scroll"
        />
      ) : null}
        ...
        {isConnected ? (
          <ConnectWalletBtn>
            <div
              ...
              onClick={
                chainId !== 534352
                  ? () => switchChain({ chainId: 534352, connector })
                  : handleBridgeTokens
              }
            >
              {chainId !== 534352 ? "Switch to Scroll" : "Withdraw funds"}
            </div>
          </ConnectWalletBtn>
        ) : null}
      </div>
        ...
        <div className="MuiBox-root css-qocfh-tableBox e1de0imv0">
          {/* <div className="MuiStack-root css-l4h50r e1de0imv0">
            <svg
              ...
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="mb-3 block w-fit mx-auto MuiSvgIcon-root MuiSvgIcon-fontSizeMedium css-1cm5l1a e1de0imv0"
              focusable="false"
              aria-hidden="true"
            >
              <path
              ...
              ></path>
              <path
                d="M15.8801 26.8484C13.0112 29.7162 8.16159 29.7162 5.29268 26.8484L5.15945 26.7153C2.28167 23.8476 2.28167 19 5.15057 16.1234L6.95363 14.321C7.38885 13.886 8.09053 13.886 8.52575 14.321C8.96097 14.7561 8.96097 15.4575 8.52575 15.8925L6.72269 17.6948C4.70647 19.7191 4.70647 23.1195 6.72269 25.1349L6.85592 25.2681C8.87215 27.2835 12.2829 27.2746 14.2991 25.2681L16.1021 23.4569C16.5374 23.0219 17.239 23.0219 17.6743 23.4569C17.8874 23.67 18.0029 23.9452 18.0029 24.2471C18.0029 24.5401 17.8874 24.8242 17.6743 25.0373L15.8623 26.8484H15.8801Z"
                fill="#5B5B5B"
              ></path>
              <path
                d="M24.1315 24.1228C23.6963 24.5578 22.9946 24.5578 22.5594 24.1228L7.85071 9.45569C7.63754 9.24261 7.52208 8.96738 7.52208 8.66552C7.52208 8.37253 7.63754 8.08842 7.85071 7.87534C8.28593 7.4403 8.98762 7.4403 9.42284 7.87534L24.1315 22.5424C24.3447 22.7555 24.4602 23.0307 24.4602 23.3326C24.4602 23.6256 24.3447 23.9097 24.1315 24.1228Z"
                fill="#5B5B5B"
              ></path>
            </svg>
            <p className="text-[#5B5B5B] text-2xl font-bold text-center !font-sans mb-3">
              Wallet not connected
            </p>
            <p className="text-2xl text-[#5B5B5B] text-center !font-sans">
              Connect wallet to see your claimable asset
            </p>
            <div className="mt-6 grid place-items-center">
              <ScrollButton
                connect={<ConnectWalletBtn>Connect Wallet</ConnectWalletBtn>}
              />
            </div>
          </div> */}
          ...
          <div className="MuiStack-root css-l4h50r e1de0imv0">
            ...
            <p className="text-[#5B5B5B] text-2xl font-bold text-center !font-sans my-3">
              {isConnected ? "No transaction history" : "Wallet not connected"}
            </p>
